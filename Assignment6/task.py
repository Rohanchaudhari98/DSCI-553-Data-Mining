...
def read_data(rows):
    ...
    for i in uset:
        ...
        if csdistmin < ((dummy-1)*threshold)+threshold:
            ...
            cs2.update(
                {lablemin: (dummy-1+csun, csusum*dummy, ((dummy-1)*csusumsq)+csusumsq, (2*dummy) + csucent - dummy - dummy, dummy*csuvar, csustddev+(dummy-1))})
            cs.append(i)
...
if __name__ == '__main__':
	...
	while (start*dummy) < (len(data)+(dummy-1)):
	    ...
	    if (start+dummy-1) >= ((dummy-1)*len(data)) + len(data):
	        ...
	        inter.update(
	            {round: (len(ds)*dummy, (dummy-1) + len(cs2.keys()), len(cs) - (dummy-1), rs.shape[0])})
